...
- 2026-07-31: 주간 정기 점검 — 서브에이전트 3명(ai/ 핵심, ai/stock_analysis/, public/+server.js)이 병렬로 코드 리뷰 수행 후 발견된 버그·보안 이슈 중 안전하게 직접 수정 가능한 항목을 반영.
  - **Claude API**: `claude-opus-4-8` → `claude-opus-5`로 업그레이드(동일 가격대 드롭인), `analyze_stock_opinion()`에 `stop_reason=="refusal"` 가드 추가.
  - **이벤트 루프 블로킹 수정**: `main.py`의 `/chat` 일반 검색 경로(509줄 부근)와 `law_search.py`의 DDG 폴백(`search_law_ddg`)이 완전 동기 블로킹 호출을 async 핸들러 안에서 직접 실행해 다른 사용자 요청까지 멈추는 문제 발견 — `run_in_executor`로 이동(2026-06-24에 `securities_report.py`/`news_collector.py`에 이미 적용된 것과 동일 패턴이 `/chat` 본류 경로와 `law_search.py`에는 누락돼 있었음).
  - **persona 분리 회귀 재발 수정**: stock 페르소나 채팅 중 DDG 검색(`main.py` 734줄 부근)이 `run_in_executor` 호출 시 `persona_id` kwarg를 빠뜨려 2026-06-22에 고쳤던 "웹검색 결과가 hr로 고정 저장" 버그가 stock 전용 병렬수집 코드에서 재발했던 것을 발견·수정.
  - **`/history/stock/reset` no-op 수정**: DELETE 조건이 `source IN ('auto_learn','learned')`였는데 실제 저장되는 값은 `'자동학습'`/`'웹검색:URL'`이라 한 번도 매칭된 적이 없어 "초기화 완료" 메시지와 달리 데이터가 전혀 지워지지 않던 문제 수정.
  - **보안**: `/backup/download`(전체 대화이력 SQLite 덤프), `/debug/law`(내부 API 키로 외부 호출 + 트레이스백 노출), `DELETE /history`, `DELETE /history/stock/reset`이 인증 없이 열려 있던 문제에 대해 `ADMIN_TOKEN` 환경변수 기반 선택적 인증(`_require_admin`)을 추가 — 미설정 시 기존과 동일하게 열려 있어 하위호환 유지, 배포 환경에서는 설정 권장. Google Drive OAuth 토큰 pickle 파일에 `os.chmod(0o600)` 추가. 업로드 엔드포인트(`/learn/document`, `/analyze/resume`)에 20MB 크기 제한 추가.
  - **XSS**: `public/index.html`(프로젝트 기억 메모)과 `public/budget.html`(예산 원본 데이터 테이블)이 서버에서 온 문자열을 이스케이프 없이 `innerHTML`에 삽입하던 부분에 `escapeHtml()` 헬퍼 추가.
  - **주식 분석 파이프라인**: (1) `validators.py` — 재무 데이터 수집 자체가 실패한 종목이 필드가 전부 비어 있어 이상값 검사를 통과한 것처럼 "검증통과"로 표시되던 버그 → "데이터없음"으로 명확히 표시. (2) `economic_collector.py`의 `get_macro_signal()`이 어디서도 호출되지 않아 `aggregator.py`/`claude_client.py`가 읽는 최상위 "종합판단" 키가 항상 기본값(정보없음/중립)이던 문제 → `run()`에서 채워 넣도록 수정. (3) `stock_api.py`의 `POST /stock/analyze` 중복 실행 방지 플래그가 `BackgroundTasks` 콜백 내부에서만 세팅돼 거의 동시에 들어온 두 요청이 모두 통과해 파이프라인이 중복 실행될 수 있던 레이스 컨디션 수정(요청 처리 시점에 즉시 세팅). (4) `pipeline.py`의 오류 보고서가 전체 Python traceback을 사용자에게 그대로 노출하던 것을 서버 로그 안내 문구로 대체(traceback 자체는 `error_log`에 계속 보존). (5) `team_config.py`의 `/stock/team` 응답이 실제로는 구현되지 않은 DCF/EV-EBITDA/PSR/MACD를 담당업무로 표기하고 있어 실제 구현(PER/PBR + 이동평균/RSI/볼린저밴드)에 맞게 수정.
  - **접근성**: `index.html` 네비게이션 탭에 `role="tab"`/키보드(Enter·Space) 지원 추가, `budget.html` 파일 업로드 `<label for=...>` 연결.
  - **다음 논의 필요(직접 수정하지 않음, 구조적 판단 필요)**: (a) 루트 `server.js`/`budget.js`/`public/budget.html`의 예산관리 기능은 어디에도 마운트되어 있지 않은 죽은 코드이며, 실제로 쓰이는 예산관리 기능은 `ai/main.py`+`ai/budget_store.py`+`ai/static/budget.html`(FastAPI 쪽)이 별도로 존재함 — 두 구현이 병렬로 남아 있어 혼란의 소지가 있음(제거할지, Node 쪽을 실제로 연결할지 결정 필요). (b) `server.js`의 모든 쓰기 엔드포인트(고정 에이전트 변경, 프로젝트 기억 저장/삭제)에 인증이 없고 `cors()`가 기본 전체 허용이라, 배포 환경에 공개될 경우 계정 ID를 아는 누구나 데이터를 쓰거나 지울 수 있음. (c) `stock_analysis/agents/aggregator.py`가 계산한 정규화 결과가 실제 분석/검증/리포트 단계(3~5단계)에서 전혀 소비되지 않고 원본 데이터가 그대로 쓰이고 있음 — 파이프라인에 연결할지 제거할지 결정 필요.
