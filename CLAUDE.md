...
- 2026-08-04: 모바일/아이패드 타워 디펜스 게임(`public/tower-defense.html`) 신규 구현 — 타워 설치·강화(레벨 1~3)·같은 종류/등급 타워 합성(티어 I~IV)·판매, 웨이브 생존형 진행(일반/빠름/탱크 + 보스 3종 로테이션: 타이탄/광전사/군주, 군주는 방어력 스탯 도입), localStorage 기반 저장/이어하기(자동 저장 + 💾 수동 저장), 사용 가능한 아이템 시스템(골드 주머니/회복 물약/메테오/냉각폭탄, 🎒 인벤토리 패널)까지 구현. 서비스워커(`public/td-sw.js`, scope `/tower-defense.html`로 다른 페이지·`/api/*`에는 영향 없음)와 웹앱 매니페스트(`public/td-manifest.json`)를 추가해 PWA화 — 온라인 상태에서 한 번 접속 후 아이패드 홈 화면에 추가하면 완전 오프라인으로 플레이·저장·이어하기 가능(실제 오프라인 시뮬레이션으로 검증)
- 2026-08-04: 게임을 인터넷 어디서든 접속 가능하도록 배포 조사 — 저장소에 이미 `homepage: https://gpt-assistant-zeta.vercel.app` (Vercel) 설정이 있으나 현재 403(배포 보호 활성화 또는 만료 추정, Vercel 대시보드 접근 권한이 이 세션에 없어 원인 특정 불가)으로 응답해 사용 불가 확인. `index.html`/`budget.html`은 `/api/*` Express 엔드포인트에 의존해 정적 호스팅만으로는 동작 불가하지만, `tower-defense.html`은 API 의존이 전혀 없는 순수 클라이언트 코드라 GitHub Pages 같은 정적 호스팅으로 완전히 동작 가능 — `.github/workflows/deploy-pages.yml` 신설(GitHub Actions로 `public/` 폴더를 Pages에 자동 배포). Pages 소스를 "GitHub Actions"로 사용해 저장소 루트 구조(Express가 `public/`을 서빙)를 바꾸지 않고도 배포 가능(레거시 "Deploy from a branch" 방식은 `/docs` 또는 루트만 지원해 `public/` 서브폴더를 바로 쓸 수 없었음). **main은 별도 앱(추정: 위 Vercel)과 연계되어 있어 건드리지 않기로 함** — 워크플로 트리거를 main이 아닌 게임 전용 개발 브랜치(`claude/mobile-tower-defense-game-qfew2j`)의 `public/**` 푸시로 한정해, PR을 main에 병합하지 않고도 이 브랜치에 계속 푸시하는 것만으로 Pages가 자동 갱신되도록 구성. GitHub Pages(Actions 배포 방식)는 어떤 브랜치에서 워크플로가 도는지와 무관하게 그 시점 아티팩트를 그대로 배포하므로 main 병합 없이도 동작함
