...
- 2026-08-04: 게임을 인터넷 어디서든 접속 가능하도록 배포 조사 — 저장소에 이미 `homepage: https://gpt-assistant-zeta.vercel.app` (Vercel) 설정이 있으나 현재 403(배포 보호 활성화 또는 만료 추정, Vercel 대시보드 접근 권한이 이 세션에 없어 원인 특정 불가)으로 응답해 사용 불가 확인. `index.html`/`budget.html`은 `/api/*` Express 엔드포인트에 의존해 정적 호스팅만으로는 동작 불가하지만, `tower-defense.html`은 API 의존이 전혀 없는 순수 클라이언트 코드라 GitHub Pages 같은 정적 호스팅으로 완전히 동작 가능 — `.github/workflows/deploy-pages.yml` 신설(GitHub Actions로 `public/` 폴더를 Pages에 자동 배포, main 브랜치의 `public/**` 변경 시 트리거 + 수동 실행 지원). Pages 소스를 "GitHub Actions"로 사용해 저장소 루트 구조(Express가 `public/`을 서빙)를 바꾸지 않고도 배포 가능(레거시 "Deploy from a branch" 방식은 `/docs` 또는 루트만 지원해 `public/` 서브폴더를 바로 쓸 수 없었음)
