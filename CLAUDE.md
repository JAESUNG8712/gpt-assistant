...
두 개의 구독(Pro/Max) 계정을 번갈아 사용해 작업을 이어간다. 완전 자동 전환(스크립트가 알아서 계정을 순회)은 사용하지 않는다 — 사람이 한도 도달을 인지하고 직접 다른 계정으로 로그인하는 반자동 방식을 쓴다. 작업 산출물(코드)은 계정이 아니라 이 git 저장소에 저장되고, 대화 맥락은 아래 "프로젝트 기록" 섹션이 계정과 무관한 공유 메모리 역할을 하므로, 별도의 실시간 동기화 없이도 계정을 바꿔가며 이어서 작업할 수 있다.

### 절차
1. 한 계정에서 사용량 한도 도달 알림을 받으면, 하던 작업을 커밋 & 푸시한다 (미완성 상태라도).
2. 아래 포맷으로 "프로젝트 기록"에 진행 상황을 한 줄 남긴다.
3. 다른 계정으로 로그인해 같은 저장소/브랜치를 열고, "최근 커밋과 프로젝트 기록의 핸드오프 메모를 확인하고 이어서 진행해줘"라고 지시한다.
4. 다시 한도에 도달하면 1~3을 반대 계정으로 반복한다.

### 핸드오프 메모 포맷
`- YYYY-MM-DD 핸드오프: 완료 — (지금까지 한 일) / 다음 단계 — (이어서 할 일) / 막힌 점 — (있다면)`
