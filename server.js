...
각 항목에 대해 원문에서 그 재직기간·업무와 짝이 맞는, 지원자가 실제로 "재직/소속"했던 회사명을 찾아 제시해라.

**중요 — 발주처/고객사와 재직 회사를 혼동하지 마라**: 프리랜서·SI 경력이 많은 이력서에서는 업무내용 문장 자체에 "삼성전기 MES 구축", "한국선급 시스템 프로젝트", "GS칼텍스 파트너시스템 프로젝트" 처럼 그 프로젝트를 발주한 고객사 이름이 등장하는 경우가 매우 흔하다. 이런 문장 속 기업명은 지원자가 프로젝트를 수행해 준 대상(고객사)일 뿐, 지원자가 소속되어 급여를 받은 회사가 아니다. 재직 회사명은 보통 그 경력 항목의 재직기간·업무 바로 옆이나 위쪽에 별도의 짧은 회사명 형태(예: "㈜", "주식회사", "…기술", "…정보", "…솔루션", 영문 약칭 등)로 독립적으로 등장한다. 업무내용 문장 안에서 그럴듯한 기업명을 발견했다고 그것을 회사명으로 쓰지 말고, 반드시 원문 다른 위치에서 그 항목의 "재직 회사명" 자체가 별도로 언급된 경우에만 답하라.

원문에서 확실히 찾을 수 없거나 확신이 없으면 절대 추측해서 지어내지 말고 반드시 빈 문자열로 남겨라. 특히 위에서 설명한 고객사/발주처 이름을 회사명 대신 넣는 것은 틀린 답이니 하지 마라.
