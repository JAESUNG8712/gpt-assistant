...
//
// SheetJS는 보안 패치된 0.20.3을 사용하지만, 손상되거나 비정상적으로 복잡한 파일이
// 메인 이벤트 루프를 오래 점유하지 못하도록 파싱을 워커 스레드로 격리하고 타임아웃을 뒀다. 이
// 완화책이 실제로 "그 요청만 실패하고 서버 전체는 계속 살아있다"는 목적을 달성하는지가
// 이 테스트의 핵심 관심사다 — 악성 페이로드 없이도 PARSE_SHEET_TIMEOUT_MS를
