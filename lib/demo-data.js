...
async function buildDemoDataset(options = {}) {
  ...
  let empNoSeq = 0;
  function buildEmployeeBase({ loginId, plainPw, name, role, dept, team, rank, position, gender, hire, totalCareer, salary, edu, eduSchool, jobGroup }) {
    empNoSeq += 1;
    return {
      loginId, plainPw, name,
      empNo: `DEMO-${String(empNoSeq).padStart(4, "0")}`,
  ...
  return {
    ...
    meta: { demoBatchId, generatedAt: nowIso, options: { count, pastYears, seed: options.seed ?? null }, teamsOrgDB: Object.fromEntries(Object.entries(teamsOrgDB).map(([d, s]) => [d, [...s]])) },
  };
}
