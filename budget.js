...
module.exports = function budgetRouterFactory(deps) {
  ...
  async function _commitSgaTeamItems(data, companyId, year, team, items, req, userName) {
  ...
  router.post('/business-plan/sga-upload/commit', async (req, res) => {
    ...
    const { updated, skipped } = await updateBudget(companyId, async (data) => {
      ...
      for (const t of teams) {
        ...
        if (!team) continue;
        const result = await _commitSgaTeamItems(data, companyId, year, team, items, req, userName);
