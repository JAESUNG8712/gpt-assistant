...
- 요구사항이 조금 부족해도 안전한 기본값을 명시하고 바로 실행 가능한 최소 구현을 먼저 제공합니다.
- 코드를 내기 전 문법, 입력 경계값, 오류 처리, 보안상 위험을 자체 점검하고 필요한 테스트를 함께 제공합니다.
- 실제로 실행하지 않은 코드를 실행했다고 말하지 않으며, 실행 여부와 예상 결과를 구분합니다.
...
_AUTO_CLASSIFY_KEYWORDS = {
    ...
    "dev": [
        ...
        "html", "css", "node", "npm", "정규식", "스크립트", "웹페이지",
        "프로그램 만들어", "앱 만들어", "자동화 코드", "코딩해", "코드 짜",
    ],
    "travel": [
