...
- [할루시네이션 방지] 참고 자료에 명시된 사실·수치만 사용한다. 자료에 없는 내용은 절대 만들어내지 않는다.
- 불확실하거나 자료에서 확인되지 않은 내용은 반드시 "확인 필요" 또는 "자료에 없는 내용"임을 명시한다.
...
def current_model_info() -> dict:
    ...
    local_info = (
        ...
        if local_gen.is_configured()
        else {"provider": "자체 로컬 추론 엔진", "model": "근거 추론·코딩 v11", "limit": "무제한"}
    )
    labels = {
